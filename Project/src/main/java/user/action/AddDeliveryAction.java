...
public class AddDeliveryAction implements Action{
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        System.out.println("chkDefault: " + chkDefault);
        System.out.println("deli_request: " + deli_request);
        System.out.println("-----------------------------------------");
        ...
        if (cnt > 0) {
            ...
            request.setAttribute("message", "배송지 정보가 성공적으로 추가되었습니다.");
        } else {
            request.setAttribute("success", false);
            request.setAttribute("message", "배송지 추가에 실패했습니다. 다시 시도해주세요.");
        }
