...
<script>
  ...
  document.getElementById("dateSearchBtn").addEventListener("click", function () {
    ...
    let filteredTotal = 0; // 필터링된 총 매출

    rows.forEach(row => {
      // 주문일 형식 확인 및 변환 (예: YYYY/MM/DD → YYYY-MM-DD)
      const orderDateText = row.children[1].textContent.trim().replace(/\//g, "-");
      const orderDate = new Date(orderDateText + "T00:00:00");

      // 정산 금액 파싱
      const finalPriceText = row.children[5].textContent.trim().replace("₩", "").replace(/,/g, "");
      const finalPrice = Number(finalPriceText) || 0;

      console.log("주문일:", orderDateText, "정산 금액:", finalPrice);

      // 날짜 비교 및 필터링
      if (orderDate.getTime() >= startDate.getTime() && orderDate.getTime() <= endDate.getTime()) {
        row.style.display = "";  // 표시
        filteredTotal += finalPrice;  // 매출 합산
      } else {
    ...
    // 총 매출 업데이트
    console.log("최종 계산된 매출:", filteredTotal);
    document.getElementById("totalSalesAmount").textContent = `₩ ${filteredTotal.toLocaleString()}`;

    // 데이터가 없는 경우 메시지 표시
    if (filteredTotal === 0) {
      alert("선택한 기간에 해당하는 매출 데이터가 없습니다.");
    }
  });
