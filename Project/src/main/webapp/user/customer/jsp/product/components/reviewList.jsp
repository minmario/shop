        ...
        <c:forEach var="item" items="${requestScope.reviews}">
            <div class="review-item">
                <div class="review-user">
                    <p class="user-name">${item.c_name}</p>
                    <p class="review-date">${item.write_date}</p>
                </div>
                ...
                <div class="user-info">
                    <c:if test="${item.c_gender eq '1'}">
                        남성
                    </c:if>
                    <c:if test="${item.c_gender eq '2'}">
                        여성
                    </c:if>
                    <c:if test="${item.c_height ne null}">
                        &nbsp;·&nbsp;${item.c_height}cm
                    </c:if>
                    <c:if test="${item.c_weight ne null}">
                        &nbsp;·&nbsp;${item.c_weight}kg
                    </c:if>
                    <c:if test="${item.i_option_name ne null}">
                        &nbsp;|&nbsp;${item.i_option_name}&nbsp;구매
                    </c:if>
                </div>
                <div class="review-image-container">
                    <c:if test="${item.additional_images ne null}">
